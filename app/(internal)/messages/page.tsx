...
import { Search, Info, Paperclip, MoreVertical, Phone, Video, X, ShieldCheck, Mail, Globe } from "lucide-react";
...
function MessagesContent() {
  ...
  // Define contacts with a sortable timestamp
  ...
  // Sync active contact if query param changes
  useEffect(() => {
  ...
  const [chatHistories, setChatHistories] = useState<Record<string, any[]>>({
    ...
    "sc": [
       { id: 1, sender: "SC", name: "Dr. Sarah Chen", time: "Tuesday", text: "Algorithm verification complete. Ready for deployment.", isMe: false, attached: null }
    ]
  });
  ...
  const handleSend = () => {
    ...
    setChatHistories(prev => ({
      ...prev,
      [activeContactId]: [...(prev[activeContactId] || []), newChat]
    }));
    
    setContacts(prev => prev.map(c => 
      c.id === activeContactId 
        ? { ...c, lastMessageAt: timestamp, time: timeString, preview: `You: ${message}` } 
        : c
    ));
        ...
        <div className="w-full md:w-72 lg:w-80 border-b md:border-b-0 md:border-r border-soft-gray flex flex-col bg-soft-gray/10 max-h-[40vh] md:max-h-none">
          <div className="p-4 border-b border-soft-gray bg-white">
            <div className="flex justify-between items-center mb-3">
              ...
              <button className="text-slate-gray hover:text-jet-black"><MoreVertical className="w-4 h-4" /></button>
            </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5">
                      ...
                      <span className={`text-[10px] ${isActive ? 'text-emerald-lime font-bold' : 'text-slate-gray'}`}>{c.time}</span>
                    </div>
                    <p className="text-xs truncate text-slate-gray">
                      {c.preview}
                    </p>
                  </div>
                </motion.div>
          ...
          <div className="p-4 bg-white">
            <div className="border border-jet-black/20 rounded focus-within:border-emerald-lime focus-within:ring-1 focus-within:ring-emerald-lime bg-white flex flex-col transition-all shadow-sm">
              <textarea 
                ...
                className="w-full max-h-32 min-h-[3.5rem] px-4 py-3 text-[15px] text-jet-black focus:outline-none resize-none bg-transparent"
                rows={1}
              />
              <div className="flex justify-between items-center px-1.5 py-1.5 bg-soft-gray/20 border-t border-soft-gray/50 rounded-b">
                ...
                <Button 
                  ...
                  className="py-1.5 px-6 text-xs rounded shadow-none bg-emerald-lime hover:bg-emerald-lime/90 text-white font-bold disabled:opacity-50 tracking-wide"
                >
                  Send Message
                </Button>
              </div>
